...
$ci = $ci -replace '0\.2\.0-ci', '0.3.0-ci'
$ci = $ci -replace '& \$python.Source -m py_compile \$_.FullName', '& $python.Source -c "import ast, pathlib, sys; ast.parse(pathlib.Path(sys.argv[1]).read_text(encoding=''utf-8''))" $_.FullName'
