...
if [ "$DRY_RUN" -eq 1 ]; then
  echo "Dry run for Cline_Env_Mac_Admin at $ROOT_PATH"
...
{"schemaVersion":2,"environment":"Cline_Env_Mac_Admin","status":"ok","version":"0.5.0","user":"${USER:-unknown}","host":"$(hostname 2>/dev/null || uname -n)","rootPath":"$ROOT_PATH","providerConfigurationChanged":false}
EOF
echo "Initialization completed for Cline_Env_Mac_Admin."
