...
if ($DryRun) {
    Write-Host "Dry run for Cline_Env_Windows_Admin at $RootPath"
...
$status = [ordered]@{ schemaVersion = 2; environment = "Cline_Env_Windows_Admin"; status = "ok"; version = "0.5.0"; user = $env:USERNAME; domain = $env:USERDOMAIN; host = $env:COMPUTERNAME; rootPath = $RootPath; repair = [bool]$Repair; updatedAt = (Get-Date).ToString("o"); providerConfigurationChanged = $false }
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $statusPath) | Out-Null
$status | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $statusPath -Encoding UTF8
Write-Host "Initialization completed for Cline_Env_Windows_Admin."
